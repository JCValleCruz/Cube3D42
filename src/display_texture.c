...
/*   Updated: 2025/01/29 14:51:00 by jormoral         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_texture(t_cube *file, int i)
{
	...
	file->ceiling_color = rgb_to_rgba(file->rgbceilingcolour);
	file->floor_color = rgb_to_rgba(file->rgbfloorcolour);
	(void)color;
	c = 0;
	start = (HEIGHT - file->scale * 2 * 212) / 2;
	end = (HEIGHT + file->scale * 2 * 212) / 2;
	while (c < start)
		mlx_put_pixel(file->img, i, c++, file->ceiling_color);
	...
	while (c < HEIGHT)
		mlx_put_pixel(file->img, i, c++, file->floor_color);
}
